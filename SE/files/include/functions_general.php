...
/* $Id: functions_general.php 278 2009-12-14 20:43:46Z john $ */
...
function todays_recipe()
{
	...
	$sql = "
	        ...
	        ORDER BY recipe_datecreated DESC LIMIT 1
		   ";
	$recipe_info = "";
	...
	{
		$recipe_info = $database->database_fetch_assoc($resource);
		if(!empty($recipe_info["recipe_photo"])) {
		    	$recipe_info["recipe_photo"] = "./uploads_recipes/".$recipe_info["recipe_photo"];
		}
	}
	return $recipe_info;
}

function todays_poll()
{
	global $database;
	$sql = "
	        SELECT
        	...
        	INNER JOIN
        		se_users
        	ON se_users.user_id = se_polls.poll_user_id 
	        ORDER BY poll_datecreated DESC LIMIT 1
		   ";
	$poll_info = "";
	$resource = $database->database_query($sql) or die($database->database_error()."<br /><b>SQL:</b> $sql");
	...
	if( $database->database_num_rows($resource))
	{
		$poll_info = $database->database_fetch_assoc($resource);
	}
	return $poll_info;
}
