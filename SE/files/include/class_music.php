...
/* $Id: class_music.php 59 2009-02-13 03:25:54Z john $ */
...
class se_music
{
	...
	function music_list_total($start=NULL, $limit=NULL, $sort_by=NULL, $where=NULL)
  {
	...
	function music_total($where=NULL)
  {
		global $database;
		
    $sql = "SELECT NULL FROM se_music";
    
    if( $where || $this->user_id ) $sql .= " WHERE ";
    if( $this->user_id ) $sql .= " music_user_id='{$this->user_id}' ";
    if( $where && $this->user_id ) $sql .= " && ";
    if( $where ) $sql .= " {$where} ";
    
		$music = $database->database_query($sql);
