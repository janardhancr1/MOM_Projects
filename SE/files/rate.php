...
if(isset($_GET['object_table'])) { $object_table = $_GET['object_table']; } else { $object_table = "se_ratings"; }
if(isset($_GET['object_primary'])) { $object_primary = $_GET['object_primary']; } else { $object_primary = "rating_id"; }
...
// RETRIEVE RATING ROW
$rating_query = $database->database_query("SELECT * FROM se_ratings WHERE rating_object_table='".$object_table."' AND rating_object_primary='".$object_primary."' AND rating_object_id='".$object_id."'");
...
if($task == "rate" && $rating_allowed != 0 && $rating <= $max_rating && $rating >= 0) {
  ...
  if($rating_info[rating_id] == 0) {
    ...
    $database->database_query("INSERT INTO se_ratings (rating_object_table, rating_object_primary, rating_object_id, rating_value, rating_raters, rating_raters_num) VALUES ('".$object_table."', '".$object_primary."', '".$object_id."', '$new_rating', '$new_raters', '$new_total_ratings')");
  ...
  // UPDATE RATING ROW
    ...
    $database->database_query("UPDATE se_ratings SET rating_value='$new_rating', rating_raters_num='$new_total_ratings', rating_raters='$new_raters' WHERE rating_id='$rating_info[rating_id]'");
  }

  // CLEAN UP RATING TABLE (CURRENT OBJECT TYPE ONLY)
  $database->database_query("DELETE FROM r USING se_ratings r LEFT JOIN ".$object_table." t ON t.".$object_primary."=r.rating_object_id WHERE t.".$object_primary." IS NULL AND r.rating_object_table='".$object_table."' AND r.rating_object_primary='".$object_primary."'");
