<?php

$page = "more_polls";
include "header.php";

if(isset($_POST['p'])) { $p = $_POST['p']; } elseif(isset($_GET['p'])) { $p = $_GET['p']; } else { $p = 1; }
if(isset($_POST['s'])) { $s = $_POST['s']; } elseif(isset($_GET['s'])) { $s = $_GET['s']; } else { $s = "dd"; }

if(isset($_GET['type'])) { $polltype = $_GET['type']; } else { $polltype = "todays"; }

// DISPLAY ERROR PAGE IF USER IS NOT LOGGED IN AND ADMIN SETTING REQUIRES REGISTRATION
if( (!$user->user_exists && !$setting['setting_permission_poll']) || ($user->user_exists && (1 & ~(int)$user->level_info['level_poll_allow'])) )
{
  $page = "error";
  $smarty->assign('error_header', 639);
  $smarty->assign('error_message', 656);
  $smarty->assign('error_submit', 641);
  include "footer.php";
}



// SET PRIVACY LEVEL AND WHERE CLAUSE
$privacy_max = $owner->user_privacy_max($user);
$where = "(poll_privacy & $privacy_max)";

// CREATE POLL OBJECT
$entries_per_page = 10;
$poll = new se_poll($owner->user_info['user_id']);

if($polltype == "todays")
{
	$polltype_header = "Today's Polls";
	
	// GET TOTAL ENTRIES
	$total_polls = $poll->poll_total($where);
	
	// MAKE ENTRY PAGES
	$page_vars = make_page($total_polls, $entries_per_page, $p);
	
	// GET ENTRY ARRAY
	$polls = $poll->poll_list($page_vars[0], $entries_per_page, "poll_datecreated DESC", $where);
}
else if($polltype == "popular")
{
	$polltype_header = "Most Popular Polls";
	
	// GET TOTAL ENTRIES
	$total_polls = $poll->poll_total($where);
	
	// MAKE ENTRY PAGES
	$page_vars = make_page($total_polls, $entries_per_page, $p);
	
	// GET ENTRY ARRAY
	$polls = $poll->poll_list($page_vars[0], $entries_per_page, "poll_views DESC", $where);
	
}



// GET RECENT SIGNUPS
$signup_array = recent_signups();
$smarty->assign_by_ref('signups', $signup_array);

// GET RECENT POPULAR USERS (MOST FRIENDS)
$friend_array = popular_users();
$smarty->assign_by_ref('friends', $friend_array);

$smarty->assign('todays_polls', $polls);
$smarty->assign('polltype', $polltype);
$smarty->assign('polltypeheader', $polltype_header);
$smarty->assign('s', $s);
$smarty->assign('d', $d);
$smarty->assign('t', $t);
$smarty->assign('c', $c);
$smarty->assign('search', $search);
$smarty->assign('total_polls', $total_polls);
$smarty->assign('p', $page_vars[1]);
$smarty->assign('maxpage', $page_vars[2]);
$smarty->assign('p_start', $page_vars[0]+1);
$smarty->assign('p_end', $page_vars[0]+count($polls));

include "footer.php";
?>