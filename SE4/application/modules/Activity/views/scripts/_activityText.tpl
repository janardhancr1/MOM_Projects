 ...
 * @version    $Id: _activityText.tpl 6590 2010-06-25 19:40:21Z john $
 * @author     Jung
 */
?>
...
<?php
  ...
  foreach( $actions as $action ): // (goes to the end of the file)
    $subject_gid = $action->subject_type . '_' . $action->subject_id;
    if (empty($usercounts[$subject_gid]))
    {
      $usercounts[$subject_gid] = 1;
    }
    else
    {
      $usercounts[$subject_gid]++;
    }
    if ($usercounts[$subject_gid] > $this->user_limit)
    {
      continue;
    }
    try { // prevents a bad feed item from destroying the entire page
      ...
      if( $action->type == 'friends' )
      {
        ...
        if( in_array($id, $friend_requests) || in_array($rev_id, $friend_requests) )
        {
          continue;
        }
        else
  ...
  <?php if( !$this->noList ): ?><li id="activity-item-<?php echo $action->action_id ?>"><?php endif; ?>
    ...
    <div class='feed_item_body'>
      ...
      <?php
        ...
        if( $action->attachment_count > 0 && $attachment->item ):
      ...
      <div class='feed_item_date feed_item_icon <?php echo $icon_type ?>'>
        ...
        <?php if( $action->getTypeInfo()->commentable && $this->viewer()->getIdentity() ): ?>
