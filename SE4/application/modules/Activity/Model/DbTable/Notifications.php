 ...
 * @version    $Id: Notifications.php 6505 2010-06-22 23:27:39Z shaun $
 * @author     John
 */
...
class Activity_Model_DbTable_Notifications extends Engine_Db_Table
{
          ...
          Core_Model_Item_Abstract $object, $type, array $params = null)
  {
    ...
    {
      Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'notify_' . $type, array_merge(array(
      ...
      /*
      $text = Engine_Api::_()->getApi('settings', 'core')->activity_notifications_template;
      if( $text )
      {
        ...
        $mail
          ->addTo($user->email)
          ->setSubject($type)
          ->setReplyTo(Engine_Api::_()->getApi('settings', 'core')->getSetting('core.email.from', 'email@domain.com'))
          ->setBodyHtml('<html><head><base href="http://'.$_SERVER['HTTP_HOST'].'"></head><body>'.nl2br($text).'</body></html>')
          //->setBodyText($text)
          ;
  ...
  {
    return Zend_Paginator::factory($this->select()->where('user_id = ?', $user->getIdentity())->order('date DESC'));
  }
