 ...
 * @version    $Id: MessagesController.php 6519 2010-06-23 01:41:45Z shaun $
 * @author     John
 */
...
class Messages_MessagesController extends Core_Controller_Action_User
{
  ...
  {
    throw new Engine_Exception('deprecated');
    /*
    $this->view->form = $form = new Messages_Form_Reply();
    $viewer = $this->_helper->api()->user()->getViewer();
    ...
    try
    {
      ...
      // Send notifications
      foreach( $this->_helper->api()->messages()->getConversationRecipients($values['conversation']) as $user )
      {
        if( $user->getIdentity() == $viewer->getIdentity() )
        {
          continue;
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification(
          ...
          'message_new',
          array(
            'url1' => array('route' => 'messages_general', 'action' => 'view', 'id' => $values['conversation']),
