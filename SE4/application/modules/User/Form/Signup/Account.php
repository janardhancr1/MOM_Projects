 ...
 * @version    $Id: Account.php 6590 2010-06-25 19:40:21Z john $
 * @author     John
 */
...
class User_Form_Signup_Account extends Engine_Form
{
  public function init()
  {
    ...
    $this->addElement('Text', 'email', array(
      ...
      if ($settings->getSetting('user.signup.inviteonly') > 0)
      {
        ...
        {
          $this->code->setValue($invite_code);
        }
      }
      ...
      //'onblur' => 'var el = this; en4.user.checkEmailTaken(this.value, function(taken){ el.style.marginBottom = taken * 100 + "px" });'
  ...
  public function checkInviteCode($value, $emailElement)
  {
    ...
    if ($settings->getSetting('checkemail'))
    {
      $email = $emailElement->getValue();
    }
    else
    {
      $email =  Zend_Controller_Front::getInstance()->getRequest()->getParam('email');
    }
    $invite_table =  Engine_Api::_()->getDbtable('invites', 'invite');
    $row = $invite_table->fetchRow($invite_table->select()->where('recipient = ?', $email));
