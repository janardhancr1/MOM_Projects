...
class User_Api_Core extends Core_Api_Abstract
{
  ...
  public function getUser($identity)
  {
    ...
    // Create new instance
    $user = new User_Model_User($identity);
  ...
  public function getAuthAdapter()
  {
    if( is_null($this->_authAdapter) )
    {
      ...
      $salt = $this->api()->getApi('settings', 'core')->getSetting('core.secret', 'staticSalt');

      $this->_authAdapter = new Zend_Auth_Adapter_DbTable(
        $db,
        $this->api()->getDbtable('users')->info('name'), // Change this to get from users model?
        'email',
        'password',
