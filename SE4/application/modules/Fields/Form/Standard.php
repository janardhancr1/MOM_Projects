 ...
 * @version    $Id: Standard.php 6527 2010-06-23 02:03:14Z jung $
 * @author     John
 */
...
class Fields_Form_Standard extends Engine_Form
{
  ...
  public function saveValues()
  {
    ...
    /*
    $aliasValues = Engine_Api::_()->getApi('core', 'fields')->getFieldsValuesByAlias($this->getItem());
    $searchRow = Engine_Api::_()->getApi('core', 'fields')->getFieldsSearch($this->getItem());
    if( $searchRow === null )
    {
      $searchRow = Engine_Api::_()->getApi('core', 'fields')->getTable($this->getItem()->getType(), 'search')->createRow();
      $searchRow->item_id = $this->getItem()->getIdentity();
    }
    $searchRow->setFromArray($aliasValues);
    $searchRow->save();
    */
  ...
  public function isValid($data)
  {
    ...
    foreach ($this->getElements() as $key => $element) {
      ...
      if( !empty($parent_id) ) {
        ...
        // specified, or field is a heading
