...
class Network_Model_Network extends Core_Model_Item_Abstract
{
	...
	public function recalculateNew(User_Model_User $user)
	{
		if( $this->membership()->isMember($user) ) {
			$this->membership()->removeMember($user);
		}
		...
		foreach($values as $index => $info)
		{
			...
			{
				$value = $info->value;
				if($value == "CA" && $this->title == "Canadian")
				{
					if( !$this->membership()->isMember($user) ) {
						$this->membership()->addMember($user)
						->setUserApproved($user, true)
				...
				elseif ($value == "US" && $this->title == "American")
				{
					if( !$this->membership()->isMember($user) ) {
						$this->membership()->addMember($user)
						->setUserApproved($user, true)
				...
				else if($value != "US" && $value != "CA" && $this->title == "International")
				{
					if( !$this->membership()->isMember($user) ) {
						$this->membership()->addMember($user)
						->setUserApproved($user, true)
