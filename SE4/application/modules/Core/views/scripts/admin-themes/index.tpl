 ...
 * @version    $Id: index.tpl 6657 2010-07-01 01:38:38Z john $
 * @author     Jung
 */
?>
            ...
            <?php foreach( $this->themes as $theme ):?>
              ...
              if ($theme->name === $this->activeTheme->name): ?>
                ...
                <div class="theme_selected_info">
                  ...
                  <?php
                    if ( !empty($this->manifest[$theme->name]['package']['version'])) {
                      echo '<h4 class="version">v'.$this->manifest[$theme->name]['package']['version'].'</h4>';
                    }
                    if ( !empty($this->manifest[$theme->name]['package']['meta']['author'])) {
                      echo '<h4>by '.$this->manifest[$theme->name]['package']['meta']['author'].'</h4>';
                    }
                  ?>
      ...
      <ul class="admin_themes">
        ...
        <li <?php echo ($alt_row) ? ' class="alt_row"' : "";?>>
          ...
          <div class="theme_chooser_info">
                  ...
                  <?php
                  if ( !empty($this->manifest[$theme->name]['package']['version'])) {
                          echo '<span class="version">v'.$this->manifest[$theme->name]['package']['version'].'</span>';
                  }
                  if ( !empty($this->manifest[$theme->name]['package']['meta']['author'])) {
                          echo '<span class="author">by '.$this->manifest[$theme->name]['package']['meta']['author'].'</span>';
                  }
                  ?>
                  <?php if ($theme->name !== $this->activeTheme->name):?>
                          <form action="<?php echo $this->url(array('action' => 'change')) ?>" method="post">
                                  <button class="activate_button">Activate Theme</button>
                                  <?php echo $this->formHidden('theme', $theme->name, array('id'=>'')) ?>
                          </form>
                  <?php else:?>
                          <div class="current_theme"><?php echo $this->translate("(this is your current theme)") ?></div>
                  <?php endif;?>
          </div>
        </li>
        <?php $alt_row = !$alt_row; ?>
