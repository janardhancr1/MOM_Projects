 ...
 * @version    $Id: AdminContentController.php 6657 2010-07-01 01:38:38Z john $
 * @author     John
 */
      ...
      foreach( $strucure as $element ) {
        ...
        else
        {
          ...
          if( !empty($parentContentRow) ) {
            ...
            var_dump($content_id);
            var_dump($tmp_content_id);
            var_dump($parent_id);
            var_dump($tmp_parent_id);
            var_dump($contentRow->toArray());
            var_dump($parent_id);
            var_dump($newRowsByTmpId);
            die();
          } */
  ...
  public function widgetAction()
  {
    ...
    if( !empty($contentInfo[$name]['adminForm']) ) {
      ...
      if( !$form->getElement('submit') ) {
        $form->addElement('Button', 'Save Changes', array('type' => 'submit'));
      }
