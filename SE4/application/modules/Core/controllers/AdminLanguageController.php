 ...
 * @version    $Id: AdminLanguageController.php 6657 2010-07-01 01:38:38Z john $
 * @author     Jung
 */
...
class Core_AdminLanguageController extends Core_Controller_Action_Admin
{
  ...
  public function indexAction()
  {
    ...
    foreach( $languageList as $localeCode ) {
      $languageNameList[$localeCode] = Zend_Locale::getTranslation($localeCode, 'language');
  ...
  public function translateAction()
  {
    ...
    $source = $this->_getParam('source', 'en');
    $sourceDir = $this->_languagePath . '/' . $source;
    if( !$source || !Zend_Locale::isLocale($source) || !is_dir($sourceDir) ) {
      throw new Engine_Exception('invalid source locale');
    }

    // Make sure target is valid
    $target = $this->_getParam('target');
    $targetDir = $this->_languagePath . '/' . $target;
    if( !$target || !Zend_Locale::isLocale($target) || !is_dir($targetDir) ) {
      throw new Engine_Exception('invalid target locale');
    }
    ...
    foreach( $files as $file ) {
      ...
      while( false != ($tmp = each($messages)) || !$break ) {
        ...
        if( count($currentKeys) < 10 && $tmp ) {
          ...
          if( is_array($value) ) {
            foreach( $value as $val ) {
              $currentKeys[] = $key;
              $currentValues[] = $this->_escapeSprintf($val);
            }
          } else {
            $value = $this->_escapeSprintf($value);
            $currentKeys[] = $key;
            $currentValues[] = $value;
          }
          continue;
        ...
        // Send
        $response  = $languageApi->query($source, $target, $currentValues);
        ...
        foreach( $currentKeys as $index => $key ) {
          ...
          $value = $this->_unescapeSprintf($response[$index]);
    ...
    var_dump('Files:');
    var_dump($files);
    var_dump('Previously Untranslated:');
    var_dump($previouslyUntranslated);
    var_dump('Successfully Translated:');
    var_dump($successfullyTranslated);
    var_dump('Diff:');
    var_dump(array_diff_key($previouslyUntranslated, $successfullyTranslated));
    
    die('translation complete');
  }
...
echo "<pre>"; print_r($languages);exit;
  ...
  {
    $string = preg_replace_callback('/(\%(?:[0-9]\$)[ds])/', array($this, '_escapeSprintfInternal'), $string, -1, $count);
    if( $count ) {
      //var_dump($string);
    }
    return $string;
  ...
  protected function _escapeSprintfInternal($matches)
  {
    $sample = array('d', 's');
    $mark = $matches[0];
    ...
    if( strlen($mark) == 4 ) {
      $pos = $mark[1];
      $typ = $mark[3];
      $typ_i = array_search($typ, $sample) + 1;
      $mark = ' ' . str_repeat('|', 10 * $typ_i + $pos) . ' ';
    } else if( strlen($mark) == 2 ) {
      $pos = 0;
      $typ = $mark[1];
      $typ_i = array_search($typ, $sample) + 1;
      $mark = ' ' . str_repeat('|', 10 * $typ_i + $pos) . ' ';
  ...
  {
    $string = preg_replace_callback('/[ ]*\|+[ ]*/', array($this, '_unescapeSprintfInternal'), $string, -1, $count);
    if( $count ) {
      //var_dump($string);
    }
    return $string;
  ...
  protected function _unescapeSprintfInternal($matches)
  {
    $sample = array('d', 's');
    
    $count = strlen(trim($matches[0]));
    $pos = $count % 10;
    $typ_i = floor($count / 10);

    if( !isset($sample[$typ_i - 1]) ) {
      return ' WHOOPS(' . $matches[0] . ') '; // For debugging
      return $matches[0]; // To be on the safe side
    }

    $typ_str = $sample[$typ_i - 1];

    if( $pos == 0 || $pos == '0' ) {
      $mark = '%' . $typ_str;
    } else {
      $mark =  '%' . sprintf('%d', $pos) . '$' . $typ_str;
    }
