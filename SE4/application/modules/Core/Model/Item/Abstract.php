 ...
 * @version    $Id: Abstract.php 6590 2010-06-25 19:40:21Z john $
 * @author     John
 */
...
abstract class Core_Model_Item_Abstract extends Engine_Db_Table_Row implements Core_Model_Item_Interface
{
  ...
  /**
   * Contains the helperbroker
   * 
   * @var Core_Model_Item_HelperBroker_Broker
   */
  protected $_helper;
  ...
  {
    return ( (!isset($this->search) || $this->search) && !empty($this->_searchColumns) && is_array($this->_searchColumns) );
  }
  ...
  protected function _postInsert()
  {
    ...
    {
      $doUpdate = false;
      foreach( $this->_searchColumns as $column )
      {
        if( !empty($this->_modifiedFields[$column]) )
        {
          $doUpdate = true;
        }
      }
  ...
  /**
   * Reads data from database into this object
   * 
   * @param mixed $spec
   * @return void
   */
  ...
  /**
   * Copies data from a database row into this object
   * 
   * @param Zend_Db_Table_Row_Abstract $row
   */
  protected function _cloneData(Zend_Db_Table_Row_Abstract $row)
  {
    $prop = $this->getShortType() . '_id';
    if( !isset($row->$prop) )
    {
      throw new Core_Model_Exception('Row to clone does not have the proper identity column');
    }
    
    // From $this->_refresh
    $this->_primary = array($prop);
