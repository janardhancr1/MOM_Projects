 ...
 * @version    $Id: GTranslate.php 6072 2010-06-02 02:36:45Z john $
 */
...
class Engine_Service_GTranslate
{
  ...
  protected $_localeToLanguage = array(
    'he' => 'iw',
  );
  ...
  public function query($from, $to, $message)
  {
    ...
    if( isset($this->_localeToLanguage[$from]) ) {
      $from = $this->_localeToLanguage[$from];
    }
    if( isset($this->_localeToLanguage[$to]) ) {
      $to = $this->_localeToLanguage[$to];
    }

    // Validate args
    if( !is_string($from) || !in_array($from, $this->_availableLanguages) ) {
      throw new Engine_Service_GTranslate_Exception('"From" language not available.');
    }
    if( !is_string($to) || !in_array($to, $this->_availableLanguages) ) {
