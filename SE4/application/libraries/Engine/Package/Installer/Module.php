 ...
 * @version    $Id: Module.php 6684 2010-07-02 00:57:13Z john $
...
class Engine_Package_Installer_Module extends Engine_Package_Installer_Abstract
{
  ...
  {
    $package = $this->_operation->getResultantPackage();
    $db = $this->getDb();
    $successCount = 0;
    $errors = array();
    ...
    if( !$this->hasError() ) {
      ...
      if( $package ) {
        $updateData = array(
          ...
          'description' => $package->getMeta()->getDescription(),
        );
      } else {
        $updateData = array(
          'version' => $this->_targetVersion,
      ...
      $this->getDb()->update('engine4_core_modules', $updateData, array(
        'name = ?' => $package->getName(),
  ...
  public function onEnable()
  {
    ...
    $db->update('engine4_core_modules', array(
      ...
      'name = ?' => $this->getOperation()->getPackage()->getName(),
    ));
  ...
  protected function _listSqlScripts()
  {
    if( null === $this->_scripts ) {
      ...
      $path = $this->_operation->getPackage()->getBasePath() . '/'
            . $this->_operation->getPackage()->getPath() . '/'
            . 'settings';
