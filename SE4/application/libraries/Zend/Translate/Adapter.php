...
abstract class Zend_Translate_Adapter {
    ...
    public function addTranslation($data, $locale = null, array $options = array())
    {
        ...
        // @MOD: $reloadLanguageFiles will contain "custom.csv" files which need to be re-added at the very end, so as to override any existing definitions
            ...
            foreach (new RecursiveIteratorIterator(
                ...
                if ($info->isDir()) {
                    ...
                    if (($this->_options['scan'] === self::LOCALE_DIRECTORY) and (Zend_Locale::isLocale($file, true, false))) {
                        // @MOD this conditional wants only locale_YY if there is at least one; disabled -sp@webligo.com
                        if (TRUE || strlen($prev) <= strlen($file)) {
                            $locale = $file;
                            $prev   = (string) $locale;
                        }
                    }
                    ...
                    try {
                        // @MOD: store custom.csv's into an array to be added last
                        if (preg_match('~/custom\.csv$~', $info->getPathname())) {
                          $customLanguageFiles[ (string) $locale ] = $info->getPathname();
                          continue;
                        }
        ...
        // @MOD: lastly, load custom.csv's
        if (!empty($customLanguageFiles)) {
          foreach ($customLanguageFiles as $locale => $file) {
            $this->_addTranslationData($file, $locale, $this->_options);
