...
class Activity_Plugin_Core
{
  ...
  public function addActivity($event)
  {
    ...
    if( in_array($content, array('everyone', 'networks')) &&
        ...
        Engine_Api::_()->authorization()->isAllowed($object, 'owner_network', 'view') ) {
      $networkTable = Engine_Api::_()->getDbtable('membership', 'network');
    ...
    // Members
    if( $object instanceof User_Model_User ) {
      if( $object->authorization()->isAllowed('member', 'view') ) {
        $event->addResponse(array(
          'type' => 'members',
      ...
      if( $object->authorization()->isAllowed('owner_member', 'view') ||
          $object->authorization()->isAllowed('parent_member', 'view') ) {
        $event->addResponse(array(
          'type' => 'members',
          'identity' => $objectParent->getIdentity()
        ));
    ...
    // Registered
    if( $content == 'everyone' &&
        Engine_Api::_()->authorization()->isAllowed($object, 'registered', 'view') ) {
      $event->addResponse(array(
        'type' => 'registered',
        'identity' => 0
      ));
    ...
    // Everyone
    if( $content == 'everyone' &&
        Engine_Api::_()->authorization()->isAllowed($object, 'everyone', 'view') ) {
      $event->addResponse(array(
        'type' => 'everyone',
        'identity' => 0
      ));
