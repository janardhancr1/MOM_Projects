  ...
  onReconfigure : function(data) {
    ...
    if( $type(data.chat_enabled) ) {
      ...
      if( parseInt(data.chat_enabled) == 0 ) {
        ...
        (new Element('div', {
          'html' : translate_chat['The chat room has been disabled by the site admin']
...
var ChatHandler_Room = new Class({
  ...
  render : function() {
    ...
    */
    ...
    if( roomList.getKeys().length > 0 ) {
      ...
      new Element('a', {'href' : 'javascript:void(0);', 'html' : translate_chat['Browse Chatrooms']}).inject(this.elements.roomsButton);
  ...
  send : function() {
    ...
    if( this.rate.length >= this.options.rateMessages ) {
      this.onGroupChat({
        ...
        'system' : true,
        'body' : translate_chat['You are sending messages too quickly - please wait a few seconds and try again.']
      });
      return;
    }
  ...
  onJoin : function(data) {
    ...
    this.elements.headerTitle.set('html', '<h3>' + data.room.title + '</h3>');
  },
    ...
    if( parseInt(data.state) >= 1 ) {
      if( !userEl ) {
        ...
        if( !$type(data.stale) || !data.stale ) {
          this.onGroupChat({
            'system' : 1,
            'body' : '<a href="' + data.href + '" target="_blank">' + data.title + '</a>' + translate_chat[' has joined the room.'],
            'room_id' : this.options.identity
          });
        }
      ...
      if( userEl ) {
        ...
        if( !$type(data.stale) || parseInt(data.stale) != 1 ) {
          this.onGroupChat({
            'system' : 1,
            'body' : '<a href="' + data.href + '" target="_blank">' + data.title + '</a>' + translate_chat[' has left the room.'],
            'room_id' : this.options.identity
          });
        }
      }
  ...
  onListRooms : function(data) {
    ...
    $H(data).each(function(room) {
      new Element('li', {
        'html' : room.title + ' (' + room.people +' people)',
...
var ChatHandler_Whispers = new Class({
  ...
  render : function() {
    ...
    this.items.friends = new ChatHandler_Whispers_UI_Friends(this, this.elements.container, {
      'name' : 'friends',
      'uid' : 'friends',
      'title' : translate_chat['Friends Online'],
      'showClose' : false
    });
...
var ChatHandler_Whispers_UI_Settings = new Class({
  ...
  toggleOnline : function(state) {
    ...
    if( state == 0 ) {
      ...
      this.elements.tooltip.set('text', 'Open Chat');
...
var ChatHandler_Whispers_UI_Friends = new Class({
  ...
  render : function() {
    ...
    new Element('div', {
      ...
      'html' : translate_chat['None of your friends are online.']
    }).inject(this.elements.menu);
  },
